...
const STATIC_RUN_ID = 'static-three-model-showcase';
...
function ModelResults({ results, runId }) {
  const entries = Object.entries(results?.models || {});
  if (!entries.length) return null;
  return <div className="mt-6 space-y-5">{entries.map(([name, output]) => <section key={name} className="rounded-2xl border bg-white p-5"><div className="mb-4 flex flex-wrap items-center justify-between gap-4"><div className="flex flex-wrap items-center gap-3"><h2 className="text-lg font-semibold">{MODEL_LABELS[name] || name}</h2><ChartExplainButton chartId="model_report" runId={runId} modelType={name} label="Ask Gemini about report" /></div><span className="text-sm">Score {Number(output.score || 0).toFixed(3)}</span></div><div className="grid gap-5 lg:grid-cols-2"><div><h3 className="mb-2 text-sm font-medium">Current mode</h3><Report report={output.metrics?.current_mode} /></div><div><h3 className="mb-2 text-sm font-medium">Fallback mode</h3><Report report={output.metrics?.fallback_modes} /></div></div></section>)}</div>;
}
...
export default function ModelLabTab() {
  ...
  const [staticLlm, setStaticLlm] = useState('');
  ...
  const askGeminiAboutStaticResults = async () => {
    setStaticLlm('Loading Gemini analysis...');
    const body = new FormData();
    body.append('run_id', STATIC_RUN_ID);
    try {
      const response = await fetch(`${API_BASE}/api/model-analysis/llm`, { method: 'POST', body });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.detail || 'Gemini analysis failed');
      setStaticLlm(payload.analysis);
    } catch (requestError) {
      setStaticLlm(requestError.message);
    }
  };
  ...
  return <div className="h-full overflow-y-auto p-4 sm:p-8"><div className="mx-auto max-w-7xl">
    ...
    {view === 'static' ? <>
      <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><div className="flex flex-wrap items-center justify-between gap-3"><span>These are precomputed results from the default dataset. No model training or saved weights are required to explore them.</span><button type="button" onClick={askGeminiAboutStaticResults} className="rounded-lg border border-[#1a73e8] bg-white px-4 py-2 text-sm font-medium text-[#1a73e8]">Ask Gemini to compare models</button></div>{staticLlm && <div className="mt-4 whitespace-pre-wrap rounded-lg bg-white/70 p-4 leading-6">{staticLlm}</div>}</div>
      ...
      <ModelResults results={staticResults} runId={STATIC_RUN_ID} />
      {staticResults && <ModelInsightsSection insights={staticInsights} models={staticModels} selectedModel={staticModel} onModelChange={selectStaticModel} runId={STATIC_RUN_ID} />}
    </> : <>
      <LocalTrainingNote />
      ...
      <ModelResults results={results} runId={runId} />
